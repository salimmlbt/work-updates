...
const CalendarView = ({ days, events }: { days: any[]; events: any[] }) => (
      ...
      {days.map(({ date, isCurrentMonth }) => {
        ...
        return (
          <div
            key={`header-${date}`}
            className={cn(
              'w-64 flex-shrink-0 border-r last:border-r-0',
      ...
      <div className="flex h-full">
        {days.map(({ date, isCurrentMonth }) => {
          ...
          const dayEvents = events.filter(
            (e) => format(parseISO(e.date), 'yyyy-MM-dd') === dayKey
          );
...
export default function CalendarClient({
  ...
  return (
    <div className="relative h-full flex flex-col">
      {/* Fixed Header inside the calendar container */}
      <div className="sticky top-0 z-50 bg-background border-b p-4 md:p-6">
      ...
      <div className="flex-1 overflow-auto">
        <Tabs value={activeCalendarView} onValueChange={setActiveCalendarView} className="flex flex-col h-full">
          <TabsContent value="holidays" className="flex-1 flex flex-col">
            <CalendarView days={daysInMonth} events={holidayEvents} />
          </TabsContent>
          <TabsContent value="my-calendar" className="flex-1 flex flex-col">
            <CalendarView days={daysInMonth} events={myCalendarEvents} />
          </TabsContent>
          <TabsContent value="falaq-calendar" className="flex-1 flex flex-col">
            <CalendarView days={daysInMonth} events={falaqCalendarEvents} />
          </TabsContent>
        </Tabs>
      </div>
