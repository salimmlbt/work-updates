        ...
        return (
          <div key={month}>
            <button
              ...
              className="w-full flex justify-between items-center px-6 py-4 rounded-[2rem] bg-white/[0.03] border border-white/10 shadow-lg hover:bg-white/10 transition-all duration-300 group backdrop-blur-md"
            >
                ...
                {items.map((leave) => {
                  ...
                  return (
                    <div key={leave.id} className="relative group">
                      <span className={cn("absolute left-[-42px] top-6 w-3.5 h-3.5 rounded-full z-10 ring-4 ring-[#0f0f0f]", statusGlow[leave.status])} />
                      <div className="bg-white/[0.03] backdrop-blur-xl rounded-[2.5rem] p-8 border border-white/10 shadow-2xl hover:shadow-[0_0_40px_rgba(56,189,248,0.06)] transition-all duration-500 hover:-translate-y-1 group-hover:bg-white/[0.05]">
                        <div className="flex justify-between gap-6">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-4 mb-4">
                              {activeTab === 'team-requests' && leave.profiles && (
                                <Avatar className="h-10 w-10 border border-white/10 ring-4 ring-white/5 shadow-2xl">
                                  <AvatarImage src={leave.profiles.avatar_url ?? undefined} />
                                  <AvatarFallback className="text-[10px] bg-zinc-800 text-zinc-400 font-black">{getInitials(leave.profiles.full_name)}</AvatarFallback>
                                </Avatar>
                              )}
                              <span className="font-black text-white tracking-tight text-2xl uppercase">
                            ...
                            <div className="flex items-center gap-6 mt-4">
                              <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">
                                {isApproved ? 'Approved Period:' : 'Statement Range:'}
                                <span className="ml-4 text-zinc-100 font-black">
                              ...
                              <Badge variant="outline" className="text-[10px] h-7 rounded-full px-5 border-white/10 bg-white/5 text-sky-400 font-black uppercase tracking-widest">
                                {duration} {duration === 1 || duration === 0.5 ? 'Day' : 'Days'} Statement
                              </Badge>
                            </div>

                            {leave.reason && (
                              <div className="mt-6 flex items-start gap-4 text-sm text-zinc-400 bg-white/[0.01] p-5 rounded-[2rem] border border-white/5 shadow-inner">
                            ...
                            {activeTab === 'team-requests' ? (
                              leave.status === 'Pending' && (
                                <div className="flex flex-col gap-3">
                                  <Button size="sm" variant="ghost" className="text-emerald-400 hover:bg-emerald-500/10 hover:text-emerald-300 rounded-2xl h-11 px-6 border border-emerald-500/20 font-black uppercase text-[10px] tracking-widest" onClick={() => setLeaveToApprove(leave)}>
                                    <CheckCircle2 className="h-4 w-4 mr-2" /> Approve
                                  </Button>
                                  <Button size="sm" variant="ghost" className="text-rose-400 hover:bg-rose-500/10 hover:text-rose-300 rounded-2xl h-11 px-6 border border-rose-500/20 font-black uppercase text-[10px] tracking-widest" onClick={() => handleAction(leave.id, 'Rejected')}>
                              ...
                              <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                {(leave.status === 'Pending' || leave.status === 'Approved') && (
                                  <Button variant="ghost" size="sm" className="text-rose-400 hover:bg-rose-500/10 rounded-full h-11 px-8 font-black uppercase text-[10px] tracking-widest border border-rose-500/10 shadow-lg" onClick={() => handleAction(leave.id, 'Cancelled')}>
                                    <XCircle className="h-4 w-4 mr-2" /> Cancel
                                  </Button>
                                )}
                                {leave.status === 'Cancelled' && (
                                  <Button variant="ghost" size="sm" className="text-sky-400 hover:bg-sky-500/10 rounded-full h-11 px-8 font-black uppercase text-[10px] tracking-widest border border-sky-500/10 shadow-lg" onClick={() => handleAction(leave.id, 'reopen')}>
                                ...
                                {(leave.status === 'Cancelled' || leave.status === 'Rejected') && (
                                  <AlertDialog>
                                    <AlertDialogTrigger asChild>
                                      <Button variant="ghost" size="icon" className="text-zinc-600 hover:text-rose-400 hover:bg-rose-500/10 rounded-full h-12 w-12 transition-colors">
                                    ...
                                    <AlertDialogContent className="rounded-[2.5rem] bg-zinc-950 border-white/10 text-white shadow-2xl backdrop-blur-3xl">
                                      ...
                                      <AlertDialogFooter className="gap-3 mt-4">
                                        <AlertDialogCancel className="rounded-2xl bg-zinc-900 border-white/10 hover:bg-zinc-800 text-zinc-300 font-bold px-8">Cancel</AlertDialogCancel>
                                        <AlertDialogAction onClick={() => handleAction(leave.id, 'delete')} className="bg-rose-600 hover:bg-rose-700 rounded-2xl text-white font-black uppercase tracking-widest text-xs px-10">Permanently Delete</AlertDialogAction>
                                      </AlertDialogFooter>
                                    </AlertDialogContent>
                                  </AlertDialog>
                                )}
    ...
    <div className="relative flex flex-col h-full bg-transparent">
      <div className="p-8 flex justify-between items-center border-b border-white/5">
        <div>
          ...
          <p className="text-sm text-zinc-500 font-medium mt-1">Audit presence history and manage upcoming studio statements.</p>
        </div>
        {isEditor && (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-white/5 p-1.5 rounded-full border border-white/10 backdrop-blur-xl">
            <TabsList className="bg-transparent border-0 h-10 gap-1">
              ...
              <TabsTrigger value="team-requests" className="rounded-full px-8 h-8 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-zinc-950 transition-all duration-500">Team View</TabsTrigger>
            </TabsList>
          </Tabs>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 pb-32 custom-scrollbar relative">
        {isLoading ? (
          <div className="flex justify-center py-40"><Loader2 className="h-12 w-12 animate-spin text-zinc-800" /></div>
        ) : leaves.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-40 text-center bg-white/[0.01] rounded-[3rem] border-2 border-dashed border-white/5 text-zinc-700">
            <div className="bg-white/5 p-12 rounded-full mb-8 shadow-2xl">
                <FileText className="h-16 w-16 opacity-20" />
            </div>
            <p className="font-black uppercase tracking-widest text-[10px]">No leave statements on record</p>
          </div>
        ) : renderTimeline()}

        {/* PERSISTENT FLOATING ACTION BUTTON */}
        <div className="fixed bottom-10 right-10 z-[150] pointer-events-none md:absolute">
            <Button 
                size="lg" 
                onClick={() => setIsApplyDialogOpen(true)}
                className="pointer-events-auto rounded-full h-18 px-10 gap-4 shadow-[0_20px_50px_rgba(56,189,248,0.4)] bg-gradient-to-br from-sky-600 to-blue-800 hover:from-sky-500 hover:to-blue-600 transition-all active:scale-95 text-white font-black uppercase tracking-widest text-xs border-2 border-white/10 ring-4 ring-sky-500/10"
            >
                <Plus className="h-7 w-7" /> Apply for Leave
            </Button>
        </div>
      </div>
