            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.01]">
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600">Timestamp Date</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Entry</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Lunch Out</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Lunch In</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Exit</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Yield (H)</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-center">Extra</th>
                  <th className="px-6 py-5 text-sm font-bold uppercase tracking-[0.3em] text-zinc-600 text-right">Audit</th>
                </tr>
              </thead>
              <tbody>
                {monthlyData.map((item) => (
                  ...
                  >
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          "h-14 w-14 rounded-2xl flex flex-col items-center justify-center font-bold transition-all group-hover:scale-105",
                          ...
                          <span className="text-lg leading-none mt-1">{format(parseISO(item.date), 'dd')}</span>
                        </div>
                        <div className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500 group-hover:text-zinc-400 transition-colors">
                          {format(parseISO(item.date), 'EEEE')}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-6 text-center">
                      <div className="text-base font-bold text-zinc-400 font-mono tracking-tight">
                      ...
                      {item.check_in_reason && (
                        <div className="mt-4 p-6 rounded-[2rem] bg-amber-500/5 border border-amber-500/10 text-left max-w-[420px] mx-auto group-hover:bg-amber-500/10 transition-colors shadow-2xl">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-600/80">Audit Context Statement</span>
                            </div>
                            <p className="text-2xl italic font-bold text-amber-100 leading-relaxed tracking-tight">
                              "{item.check_in_reason}"
                            </p>
                        </div>
                      )}
                    ...
                    <td className="px-6 py-6 text-center text-base font-bold text-zinc-400 font-mono tracking-tight">
                      <TimeDisplay time={item.check_out} />
                    </td>
                    <td className="px-6 py-6 text-center text-base font-bold text-white tracking-tight">
                      {item.total_hours?.toFixed(2) || '0.00'}
                    </td>
                    <td className="px-6 py-6 text-center">
                      {item.extra_hours > 0 ? (
                        <span className="text-base font-bold text-sky-400 tracking-tight">+{item.extra_hours.toFixed(2)}</span>
                      ) : (
