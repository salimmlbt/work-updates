...
import { Plus, Trash2, Loader2, CalendarClock, Palette } from 'lucide-react'
      ...
      <Card className="border border-white/10 bg-white/[0.03] backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-10">
        <CardHeader className="p-0 mb-10">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setAddOpen(true)} className="rounded-full bg-sky-600 hover:bg-sky-500 font-bold">
            ...
            <Table>
              <TableHeader className="bg-white/5">
                <TableRow className="hover:bg-transparent border-white/10">
                  ...
                  <TableHead className="w-20 text-right pr-8"></TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {configs.map(config => (
                  <TableRow key={config.id} className="border-white/5 hover:bg-white/[0.02] group transition-colors">
                    ...
                    <TableCell className="text-right pr-8">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-zinc-500 hover:text-rose-400 hover:bg-rose-500/10 opacity-0 group-hover:opacity-100 transition-all" onClick={() => handleDelete(config.id)}>
